...
## Own Dataset
To create an own dataset, a couple different things need to be taken into account. For example; how do I need to split my data to create a dataset that has the geographical weighted variables and a dataset that only has the spectral values of the image. This research checks the difference in geographically weighted variables and normal spectral data, compared to onl spectral data.

## Evaluation
To evaluate the ML algorith, the accuracy can be determined. In the research a stratified systematic unaligned sampling scheme was used to ensure that the pixels were dispersed across the entire image. This is a sampling scheme used for generating error matrices. 
### Metric
The metric that was used in the research was Moran's I \cite{Moran50}. This metric calculates how similar, on averag a sample is to other nearby samples. 
